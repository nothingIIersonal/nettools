...
#define STUN_RESPONSE_MAX_SIZE 256
	...
	*(uint32_t *)(request_buf + 16) = htonl(0x01234567); // Transaction ID cont.
	...
	if ( *(uint16_t *)response_buf == htons(0x0101) ) // 0x0101 - success response
	{
		...
		// skip unnecessary attributes (type = 2 byte, length = 2 byte and value of length field)
    	for ( size_t i = STUN_HEADER_SIZE; i < STUN_RESPONSE_MAX_SIZE; i += attribute_length + 4 )
   	 	{
            attribute_type = htons(*(uint16_t *)(response_buf + i));
			attribute_length = htons(*(uint16_t *)(response_buf + i + 2));

			if (attribute_type == 0x0020) // this type of attribute indicates that it is an "XOR-MAPPED-ADDRESS"
			{
				// port -> "XOR'ing it with the most significant 16 bits of the magic cookie,
				//          and then the converting the result to network byte order"
				*local_port = ntohs(*(uint16_t *)(response_buf + i + 6));
				*local_port ^= 0x2112;

				// ipv4 -> "is computed by taking the mapped IP address in host byte order,
