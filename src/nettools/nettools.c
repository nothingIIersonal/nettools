...
int get_ip_by_domain_name(char *domain_name, struct ip_t *ips, int max_ips) {
  ...
  while (addrinfo_i != NULL && ips_counter < max_ips) {
    if (addrinfo_i->ai_family == AF_INET) {
      ...
      ips[ips_counter].ipv4.A = (uint8_t)(*((uint32_t *)tmp_addr));
      ips[ips_counter].ipv4.B = (uint8_t)(*((uint32_t *)tmp_addr) >> 8);
      ips[ips_counter].ipv4.C = (uint8_t)(*((uint32_t *)tmp_addr) >> 16);
      ips[ips_counter].ipv4.D = (uint8_t)(*((uint32_t *)tmp_addr) >> 24);
